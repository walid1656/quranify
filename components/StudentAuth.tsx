...
export const StudentAuth: React.FC<StudentAuthProps> = ({ onLoginSuccess, onBackToLanding }) => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => setAuthTab('signin'), 2000);
    } catch (err: any) {
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={authTab === 'signin' ? handleSignIn : handleSignUp} className="bg-white rounded-xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-emerald-900 mb-2">
              {authTab === 'signin' ? 'رحباً بعودتك' : 'ابدأ رحلتك'}
            </h2>
            <p className="text-gray-600 mb-6">
              {authTab === 'signin'
                ? 'أدخل بيانات حسابك للمتابعة'
                : 'سجل حساباً جديداً لتبدأ التعلم'}
            </p>
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-900">
            <p className="font-semibold mb-2">🔐 بيانات تجريبية (طالب):</p>
